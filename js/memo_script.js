...
chrome.runtime.onConnect.addListener((port)=> {
    port.onMessage.addListener((request)=>{
        ...
        if(request.connect === 'context_menus'){
            ...
            document.getElementById("memotext").style.fontSize = 10;
            port.postMessage({result:"success"});
        }else if(request.connect === 'popup'){
            document.getElementById("memotext").style.color = request.color;
            document.getElementById("memotext").style.font = request.font;
            document.getElementById("memotext").style.fontSize = request.fontSize;
            port.postMessage({result:"success"});
        }
    });
});